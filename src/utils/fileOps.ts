...
export const defaultServicesFileName = '/combinedServices';
export const defaultServicesFilePath = `${defaultInputDir}${defaultServicesFileName}.${extensionTS}`;
export const defaultOutputDir = 'out';
export const defaultServiceMocksFileName = '/mocks';
export const defaultServiceMocksFilePath = `${defaultOutputDir}${defaultServiceMocksFileName}.${extensionTS}`;
...
export function generateServicesFile(
    ...
    const inputPath =
        inputDir + `${defaultServicesFileName}.${extensionTS}` ??
        defaultServicesFilePath;
    const outputPath = outputDir
        ? outputDir + `${defaultServiceMocksFileName}.${extensionTS}`
...
export function readFilesWithExtension(
    ...
    try {
        ...
        if (content) results.push(content);
            ...
            for (const file of filteredFiles) {
                ...
                if (
                    ...
                    !filePath.includes(`${defaultServicesFileName}.${extensionTS}`)
                ) {
        ...
        createFile(
            results.join('\n'),
            dir + `${defaultServicesFileName}.${extensionTS}`,
            undefined,
            true,
        );
        return results;
...
function createFile(
    ...
    const initialContent = '// Generated using mockable\n';
